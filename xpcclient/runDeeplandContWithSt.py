...
def ctrlcol_to_elev(x):
    return x*-10

def elev_to_ctrlcol(x):
    return x/-10

def ctrlwhl_to_ail(x):
    return x*20

def ail_to_ctrlwhl(x):
    return x/20
...
def draw_plot(placeholder, data: dict, fig_loc, axs_loc, fig_cmd, axs_cmd, model_act_dist):
    with placeholder.container():
        ...
        axs_loc[0].plot((0, 19000), (0, 19000*tan(3*deg2rad)), color='k')
        axs_loc[0].plot(data['dist_m'], data['hralt'])
        axs_loc[0].scatter(data['dist_m'][-1], data['hralt'][-1], color='r')
        axs_loc[0].set_ylim((0, 700))
        axs_loc[0].set_xlim((0, 18200))
        axs_loc[0].grid()
        axs_loc[0].set_xticklabels([])
        axs_loc[0].set_ylabel('Altitude (m)')

        axs_loc[1].plot((0, 19000), (0, 0), color='k')
        axs_loc[1].plot(data['dist_m'], data['loc_m'])
        axs_loc[1].scatter(data['dist_m'][-1], data['loc_m'][-1], color='r')
        axs_loc[1].set_xlim((0, 18200))
        axs_loc[1].set_ylim((-8, 8))
        axs_loc[1].set_ylabel('Localizer Dev. (m)')
        axs_loc[1].grid()

        if model_act_dist != None:
            axs_loc[0].plot((model_act_dist, model_act_dist), (0, 700), color='gray', linestyle='dashed')
            axs_loc[1].plot((model_act_dist, model_act_dist), (-8, 8), color='gray', linestyle='dashed')

        fig_loc.align_ylabels(axs_loc[:])


        axs_cmd[0].plot(data['time_s'], data['ctrl_col'])
        axs_cmd[0].set_ylim((-0.5, 0.5))
        axs_cmd[0].set_xticklabels([])
        axs_cmd[0].set_ylabel('Elevator')
        axs_cmd[0].grid()
        sec_col = axs_cmd[0].secondary_yaxis(1.05, functions=(ctrlcol_to_elev, elev_to_ctrlcol))
        sec_col.set_ylabel(r'Deg $[^o]$')

        axs_cmd[1].plot(data['time_s'], data['throttle'])
        axs_cmd[1].set_ylim((0, 1))
        axs_cmd[1].set_xticklabels([])
        axs_cmd[1].set_ylabel('Throttle')
        axs_cmd[1].grid()

        axs_cmd[2].plot(data['time_s'], data['ctrl_whl'])
        axs_cmd[2].set_ylim((-0.2, 0.2))
        axs_cmd[2].set_xticklabels([])
        axs_cmd[2].set_ylabel('Aileron')
        axs_cmd[2].grid()
        sec_whl = axs_cmd[2].secondary_yaxis(1.05, functions=(ctrlwhl_to_ail, ail_to_ctrlwhl))
        sec_whl.set_ylabel(r'Deg $[^o]$')

        axs_cmd[3].plot(data['time_s'], data['flap_rat'])
        axs_cmd[3].set_ylim((0, 1.2))
        axs_cmd[3].set_xticklabels([])
        axs_cmd[3].set_ylabel('Flap')
        axs_cmd[3].grid()

        axs_cmd[4].plot(data['time_s'], data['sas'])
        axs_cmd[4].plot(data['time_s'], data['ias'])
        axs_cmd[4].set_ylabel('Airspeed')
        axs_cmd[4].set_ylim((120, 240))
        axs_cmd[4].grid()

        fig_cmd.align_ylabels(axs_cmd[:])
        fig_cmd.tight_layout()

        loc_col, cmd_col = st.columns(2)
...
def run(elv_model: tf.keras.Model, pla_model: tf.keras.Model,
    ...
    fig_loc, axs_loc = plt.subplots(nrows=2)
    fig_cmd, axs_cmd = plt.subplots(nrows=5)
    ...
    for param in ['time_s']+DATA_TO_PLOT:
        rec_plot[param] = []
        empty_rec_plot[param] = [0]
    draw_plot(placeholder, empty_rec_plot, fig_loc, axs_loc, fig_cmd, axs_cmd, None)
        ...
        while is_running == True:
            ...
            if (datetime.datetime.now() - lastUpdate).total_seconds() >= INTERVAL:
                ...
                if time - last_plot_update > PLOT_UPDATE_INTERVAL:
                    ...
                    if model_act_dist == None and on_model:
                        model_act_dist = data_dict['dist_m']

                    if not on_model:
                        for param in ['hralt', 'h_err', 'h_ref', 'dist_m', 'loc_m']:
                            rec_plot[param].append(data_dict[param])
                    else:
                        rec_plot['time_s'].append(plot_time)
                        for param in DATA_TO_PLOT:
                            rec_plot[param].append(data_dict[param])
                        plot_time += PLOT_UPDATE_INTERVAL

                    if time - last_plot_drawn >= PLOT_INTERVAL:
                        draw_plot(
                            placeholder, rec_plot, fig_loc, axs_loc, fig_cmd, axs_cmd, model_act_dist
                            )
                        last_plot_drawn = time
                ...
                if not on_model or len (feat) < FEATURE_WINDOW_WIDTH:
                    ...
                    print ("Waiting Until Glide Slope Intercepted")
                    lastUpdate = datetime.datetime.now()
                    continue
...
if __name__ == "__main__":
    ...
    st.write("# Neural Nets. Approach Controller")
